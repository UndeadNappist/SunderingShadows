...
{
    return ({ "paladin", "cleric", "inquisitor", "magus", "mage" });
}

string* restricted_deities(string subrace)
{
    return ({ "jarmila", "kreysneothosies", "callamir", "lysara", "kismet", "nimnavanon", "lysara", "seija", "nilith", "lord shadow", "the faceless one", "mephasm", "velik", "eucalia", "ebolek", "khyron" });
}
...
{
    string* choices = ({});
    choices += ({ "ebony", "white", "silver", "gray" });
    return choices;
}
...
{
    return (["required" : ({ "strix", }),
             "optional" : ({ "undercommon", "beast", "draconic", "giant" })]);
}
