...
int set_base_class(object obj, string choice)
{
    ...
    if(member_array(choice,classes-({"psion"})-({"psywarrior"})) == -1) { return 0; }
...
int prerequisites(object player)
{
    ...
    if (!FEATS_D->usable_feat(player, "spell focus")) {
        write("Failed spell focus.");
        return 0;
    }
    if (!FEATS_D->usable_feat(player, "martial weapon proficiency")) {
        write("Failed weapon proficiency.");
        return 0;
    }
    return 1;
}
...
mapping class_featmap(string myspec) {
    return ([ 1 : ({ "eldritch conditioning" }), 4 : ({ "eldritch warfare" }), 7 : ({ "spell critical" }), ]);
}
...
int hit_dice(object ob) { return 6; }  // hit dice rolled for hitpoints each level

int default_hitpoints(object ob) { return base_class_ob(ob)->default_hitpoints(); } // hitpoints per level above level 20

string armor_allowed(object ob) { return "fighter"; }

string weapons_allowed(object ob) { return "fighter"; }
