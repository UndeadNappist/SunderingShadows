...
string requirements() // string version, maybe we'll need this, maybe not, can remove later if not
{
    ...
    str = "Prerequisites:\n"
        "    20 Ranger levels\n"
        "    14 Wisdom Stat, before equipment modifiers\n"
...
int prerequisites(object player)
{
    ...
    if (player->query_base_stats("wisdom") < 14) {
        return 0;
    }
...
int caster_level_calcs(object player, string the_class)
{
    ...
    switch(the_class)
    {
        ...
        case "ranger":
            ...
            level += player->query_class_level("beast_master");
            return level;
...
{
    return ([ 1 : ({ "chosen animal" }), 4 : ({ "pack leader" }), 7 : ({ "bestial frenzy" }), ]);
}
