...
void create()
{
    ...
    set_release_to("/d/dagger/Torm/road/path12");
    ...
    set_door("cell door", JAILMAIN, "south", "Torm jail key");
...
int peer_out(string str)
{
    ...
    if (str == "grate" || str == "through grate") {
        ...
        tell_room(ETP, TPQCN + " steps up to the cell door and " +
                  "peers out.\n", TP);
        tell_room("/d/dagger/Torm/city/jail_main", "You hear a scratching "
                  "at the door and a glimpse of movement through the grate.\n", TP);
        write("/d/dagger/Torm/city/jail_main"->query_long());
