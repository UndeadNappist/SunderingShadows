...
void create(){
    ...
    set_items(([
		...
		({"table", "map", "maps", "navigational tools"}) : (:this_object(), "look_map":),
...
int retrieve_fun(string str){
    ...
    if(guarded){
        ...
        katerina->force_me("say Those do not belong to you, trespassers.");
        guarded = 0;
        return 1;
    }
    
    if(katerina = present("katerina", room)){
