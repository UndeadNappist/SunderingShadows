...
void create(){
   ...
   set_long("%^CYAN%^Hard at first to discern as an article of clothing, this robe is %^RESET%^ta%^BOLD%^%^BLACK%^t%^RESET%^tered %^CYAN%^and frayed almost to the point of falling apart. The %^BOLD%^pale blue %^RESET%^%^CYAN%^material it has been made from is torn and rent by %^BOLD%^%^BLUE%^win%^CYAN%^d%^BLUE%^s %^RESET%^%^CYAN%^that constantly enfold the garment and tear eternally at its tattered threads. Even in a calm environment the robe flaps as if being driven in a gale. The only thing that seems to prevent the cloth from being torn apart are the %^BOLD%^%^BLACK%^bla%^WHITE%^c%^RESET%^k%^BOLD%^%^BLACK%^ened cha%^RESET%^i%^BOLD%^%^BLACK%^ns %^RESET%^%^CYAN%^that encircle it, anchoring the robe to its wearer.%^RESET%^");
    set_lore("The crafting of a robe such as this is attributed to a greatly skilled mage, argued by many to be mad. Theodore LasRadis was a powerful wizard, said to be obsessed with the mastery of greater and greater destructive evocations. He made many such items as this, that only barely contain the dangerous forces imbued within and are highly sought by other mages of similar inclination, as well as followers of Ryorik who delight in the destructive power of the elements. It was said that his life was brought to an abrupt halt following a similar experiment gone awry, unleashing the full force of his spellcraft upon himself in an unpleasant end.");
...
int remove_it(){
    ...
    tell_object(ETO,"%^CYAN%^The stormwinds around you calm as you remove the robe.%^RESET%^");
    
    if(!ETO->query_invis()) {
        tell_room(EETO,"%^CYAN%^The robe stops blowing in its own wind as "+ETOQCN+"%^RESET%^%^CYAN%^ removes it.%^RESET%^",ETO);
    }
    return 1;
}
