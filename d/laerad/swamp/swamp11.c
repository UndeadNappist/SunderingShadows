...
void create(){
    ...
    set_exits( ([
	"north":"/d/laerad/swamp/swamp12",
	"northwest":"/d/laerad/plains/plains78",
	"west":"/d/laerad/swamp/swamp10"
    ]) );
}
void reset(){
    ::reset();
    if(!present("tree")){
//	new("/d/laerad/mon/hangman_tree")->move(this_object());
    }
    if(!present("hydra")){
	new("/d/laerad/mon/hydra6")->move(this_object());
    }
}
