...
void create()
{
    ...
    set_short("%^RESET%^%^GREEN%^Lum%^BOLD%^b%^RESET%^%^GREEN%^er"
              "%^RESET%^i%^GREEN%^ng Tr%^BOLD%^e%^RESET%^%^GREEN%^ant%^RESET%^");
    set_long("%^RESET%^%^GREEN%^Resplendent with the deep "
             "%^BOLD%^green leaves %^RESET%^%^GREEN%^of summer, "
             "this treant looks robust and healthy.  The %^ORANGE%^"
             "woody brown bark %^GREEN%^is streaked with %^RESET%^silver"
             "%^GREEN%^, marking this as a creature of ancient magic.  "
             "The treant stands well over thirty feet tall in the open "
             "air, though it seems to be able to shrink to fit into "
             "smaller places.  Its trunk is over two feet thick, while "
             "its branches spread wide, bathing a vast area in shade.  "
