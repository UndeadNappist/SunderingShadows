...
void create(){
  ::create();
    ...
    set_name("exalted dead");
    set_id(({"dead","exalted dead","a mighty ghostly warrior","warrior","ghostly warrior", "exalted_dead"}));
...
void protect()
{
    ...
    if(environment(this_object()) != environment(cast))
    {
        remove();
        return;
    }
