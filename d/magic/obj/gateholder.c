...
void defend(){
	...
	for(num =0;num < numfoes;num++){
		...
		tell_room(environment(mon),"%^BOLD%^%^RED%^The outsider attacks "+tmp[num]->query_cap_name()+"!");
	}
    return;
}
...
int cmd(string str){
	...
	if(sscanf(str, "%s to %s", who, what) != 2) return notify_fail("Syntax: command <monster> to <jump up and down>\n");
	...
	if(what[0..3] == "wear") return notify_fail("The outsider refuses.");
	if(!mon->force_me(what)) return notify_fail("You fail to command the outsider to "+what+"!\n");
	return 1;
}
