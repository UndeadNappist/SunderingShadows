//silverwater.c - for Silver Sight spell by ~Circe~
...
void create(){
   ...
   set_name("silverwatcher");
   set_property("no detect",1);
   set_id(({"SilverWatcher"}));
   set_short(0);
   set_invis();
   add_limb("body","",0,0,0);
}
...
void heart_beat()
{
   ...
   if(present("blockerx111", follower)) self_destruct();
}

dest_me(){
//   tell_room(ETO,"%^BOLD%^Dancing %^RESET%^motes %^BOLD%^of silvery "+
//      "light appear off to the side before winking out of existence.%^RESET%^",TO);
      ...
      for(i=0;i<sizeof(spells);i++){  
         if(objectp(spells[i])){
            if(!spells[i]->query_target_object()){
               if((string)spells[i]->query_spell_name() == "silver sight"){
                  if(!objectp(spells[i])) return notify_fail("Your silver sight seems to have been lost.\n");
                  tell_object(casterobj,"Something seems to be blocking your silver sight!", TP);
