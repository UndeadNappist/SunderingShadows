...
void create() {
    ...
    set_long("%^RESET%^This %^CYAN%^steel hilt %^RESET%^appears to be a remnant of a sword, long since detached from the blade that was once inset to the simple guard it bears at the top. It "
...
int attach_fun(string str) {
    ...
    if(base_name(pommel) != "/d/hm_quest/obj/pommel1") {
    ...
    ATTACH = 1;
    pommel->remove();
    set_short("%^RESET%^%^CYAN%^Steel hilt (pommel attached)%^RESET%^");
    set_long("%^RESET%^This %^CYAN%^steel hilt %^RESET%^appears to be a remnant of a sword, long since detached from the blade that was once inset to the simple guard it bears at the top. It "
"still %^BOLD%^%^WHITE%^glows %^RESET%^with a faint magical aura, despite being separated from the rest of the weapon. Stranger still, there is a depression in the center of the hilt that "
"appears to have held a %^YELLOW%^jewel %^RESET%^at one time, but it has long since been pried out. At the furthest end lies a skull-shaped pommel-stone, heavy enough to serve as a sturdy "
