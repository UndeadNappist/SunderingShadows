...
"its impressive appearance, you suspect its days of use in combat are long since over, though its %^YELLOW%^golden %^RESET%^surface still glows with a faintly magical aura. It has been detached "
...
int join_fun(string str) {
    ...
    if(base_name(hilt) != "/d/hm_quest/obj/hilt1") {
    ...
    JOIN = 1;
    hilt->remove();
    set_short("%^RESET%^%^YELLOW%^Golden blade (hilt joined)%^RESET%^");
    set_long("%^RESET%^This long, straight blade is from a sword that has seen much use, %^RESET%^%^BLUE%^notched %^RESET%^and %^RESET%^%^ORANGE%^scarred %^RESET%^along its length. Despite "
"its impressive appearance, you suspect its days of use in combat are long since over, though its %^YELLOW%^golden %^RESET%^surface still glows with a faintly magical aura. It has been attached "
"to a hilt of %^CYAN%^steel%^RESET%^, set with a skull-shaped pommel. Certainly it could be sheathed to keep it protected from the elements, if one could be found to fit it.%^RESET%^");
    set_weight(9);
    return 1;
}
