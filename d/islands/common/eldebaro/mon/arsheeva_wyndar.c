...
void die(object ob) 
{
    ...
    if(myPhase >= 5) 
    {
        ...
        foreach(object usr in users())
        {
            ...
            if((int)usr->query_hp() < (int)usr->query_max_hp()) continue;
            tell_object(usr, "%^BOLD%^%^WHITE%^Your wounds are healed as the winds renew you!%^RESET%^");
            usr->set_hp((int)usr->query_max_hp());
            continue;
        }
        return ::die(ob);
    }
