...
void create(){
   ::create();
  set_long(::query_long()+"\nYou are faced with "+
  "a %^C059%^ridge%^CRST%^, and %^C065%^fetid "+
  "water%^CRST%^ sloshes down it in a %^C023%^slow "+
  "waterfall%^CRST%^, you could %^C067%^>slide<%^CRST%^ "+
  "here, but you have no clue where you might turn up..\n");
set_exits(([
