...
void create(){
    ...
    set("aggressive", 0);
    set_long("Elendil Saralondyo is a young alchemist of some reputation. His store is known for keeping rare items in stock, and he employs elves to go out and hunt for components that are in demand. He has close cropped gold hair and clear blue eyes. He wears a simple robe that has bright shades of silver with patterns of gold sewn around it.\n\nFor a list of shop commands, type <help shop>.");
    set_gender("male");
    set_alignment(1);
    set_race("elf");
    set_class("mage");
    ...
    set_emotes(6,({
        "%^MAGENTA%^Elendil Saralondyo says%^RESET%^: You see anything you like?",
    ...
    set_emotes(20, ({
        "%^MAGENTA%^Elendil Saralondyo says:%^RESET%^ I won't stand for this. ",
    ...
    set_spells(({"fireball","lightning bolt","acid arrow", "powerword stun", "chain lightning", "magic missile"}));
    set_property("strength","spells");
    set_base_damage_type("bludgeoning"); 
    set_storage_room("/d/islands/elf/mon/alchemist_storage.c");
