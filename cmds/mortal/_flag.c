...
{
    int x, j, remaining;
    string *activeEvents, eventName, notification, eventMsg, *featureVoting, featureName, *alignments;
    ...
    if(pointerp(activeEvents = WORLD_EVENTS_D->parsable_world_events(who)))
    {
        ...
        for(x = 0;x < sizeof(activeEvents);x++)
        {
            ...
            if(sizeof(alignments = WORLD_EVENTS_D->query_event_alignments(eventName)))
                eventMsg += " (" + implode(short_alignment(alignments), ", ") + ")";
...
%^CYAN%^roleplay flag %^RESET%^
  Will fly a RP flag in your current environment. This will cause your current room to show up on the who list as a RP FLAG.
