...
void create(){
    ...
    set_description("This powerful invocation allows the warlock to travel along the edge of the plane with which they bear affinity, using it as 'shortcut' of sorts to a known destination upon the prime. Such travel will heal some of the caster's injuries along the way, given their natural bond with that particular plane. Any party-mates who are with the caster at the time are considered to be willing subjects of the spell, and will travel alongside the caster to his or her destination, if they are not pre-occupied with combat.");
    set_verbal_comp();
    set_somatic_comp();
    set_arg_needed();
    set_helpful_spell(1);
...
void spell_effect(int prof){
    ...
    switch(heritage){
        ...
        case "gloom":
            ...
            tell_room(endplace,"%^CYAN%^The air trembles and %^BOLD%^%^BLACK%^darkens %^CYAN%^perceptibly.%^RESET%^",caster);
            break;
        case "astral":
            ...
            tell_room(endplace,"%^CYAN%^The air trembles amid a susurrus of %^BLUE%^whispers%^CYAN%^.%^RESET%^",caster);
            break;
        case "infernal":
...
void move_caster(object endplace, int prof){
    ...
    // pick up any party members that are in the room with the caster on spell activation
    presentparty = ({});
    ...
    if(!sizeof(presentparty)){ // teleporting alone!
        ...
        switch(heritage) {
            ...
            case "abyssal":
                ...
                break;
            case "astral":
                tell_room(endplace,"%^CYAN%^A rift suddenly parts reality in the air before you, revealing a %^RESET%^hazy realm %^CYAN%^of %^BLUE%^endless cosmos %^CYAN%^from which "+caster->QCN+" steps. It snaps shut behind "+caster->QO+"!%^RESET%^",caster);
                break;
            case "gloom":
