...
void create()
{
    ...
    feat_desc("This feat allows a ranger to focus their knowledge and training fighting a second enemy type of their choice. The ranger will receive an additional +2 bonus to attack and damage rolls while fighting their favored enemy. A ranger also adds this bonus when studying his favored enemy. You can select from the following favored enemies: \n\n"+"%^GREEN%^"+implode(valid_choices, "\n")+"%^RESET%^\n\n\To start selection process type <second_favored_enemy> [TYPE].");
    allow_tripped(1);
...
int prerequisites(object ob)
{
    ...
    if (ob->query_class_level("ranger") < 11)
    {
        dest_effect();
        return 0;
    }
