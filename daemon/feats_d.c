...
string format_feat(string feat,object targ, object viewer) {
    ...
    if (!level || level == -1) {
        ...
        tmp = "%^BOLD%^%^YELLOW%^" + level + "%^RESET%^";
    } else if (bought_as_type_feat(feat, targ, "pact")) {
        tmp = "%^BOLD%^%^YELLOW%^" + level + "%^RESET%^";
...
void display_feats(object ob,object targ, string mytype)
{
    ...
    switch(mytype) {
      ...
      case "all": case "allowed": currentlist += SPELLFEATS; currentlist += MELEEFEATS; currentlist += GENERALFEATS; currentlist += MAGUSFEATS; currentlist += PALADINFEATS; currentlist += EPICFEATS; currentlist += PRESTIGE_FEATS; currentlist += RAGEFEATS; currentlist += LOCKFEATS; break;
      default: currentlist += SPELLFEATS; currentlist += MELEEFEATS; currentlist += GENERALFEATS; currentlist += MAGUSFEATS; currentlist += PALADINFEATS; currentlist += EPICFEATS; currentlist += PRESTIGE_FEATS; currentlist += RAGEFEATS; currentlist += LOCKFEATS; break;
    }
...
int bought_as_type_feat(string feat, object targ, string feattype) {
    ...
    case "rage":
        ...
        break;
    case "pact":
        type_feats = targ->query_pact_feats();
        break;
    case "talent":
    ...
    if (!sizeof(valid_classes)) {
        ...
        for (i = 0; i < sizeof(subset); i++) {
            ...
            switch (subset[i]) {
                //martials
            case "fighter":
                j = 1 + ((obj->query_class_level("fighter") + 1) / 2);
                /*
            ...
            case "thief":
                ...
                if(category == "talent")
                {
                    if(obj->query_class_level("thief") > 5)
                        j = 1;
                    if(obj->query_class_level("thief") > 10)
                        j = 2;
                    if(obj->query_class_level("thief") > 20)
                        j = 3;
                    if(obj->query_class_level("thief") > 30)
                        j = 4;
                }
            ...
            case "bard":
                ...
                if(category == "talent")
                {
                    if(obj->query_class_level("bard") > 10)
                        j = 1;
                    if(obj->query_class_level("bard") > 20)
                        j = 2;
                }
                else
                */
